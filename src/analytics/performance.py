...
from database.models import Movie, Genre, Rating
...
class GenrePerformanceAnalyzer:
    ...
    def analyze_movie_rankings(self) -> None:
        ...
        for i, movie in enumerate(top_movies, 1):
            ...
            print(f"{i:2d}. {movie['title']} ({movie['release_year']}) - {movie['rating']}/10")
        ...
        if top_movies and bottom_movies:
            print(f"\n QUICK STATS:")
            print(f"   Best Movie: {top_movies[0]['title']} ({top_movies[0]['rating']}/10)")
            print(f"   Worst Movie: {bottom_movies[0]['title']} ({bottom_movies[0]['rating']}/10)")
            print(f"   Rating Range: {top_movies[0]['rating'] - bottom_movies[0]['rating']:.2f} points")
    ...
    def analyze_all_genres(self) -> None:
        ...
        print(f"\nBEST PERFORMING GENRE:")
        print(f"   {best_genre['Genre']} - {best_genre['Avg Rating']}/10 ({best_genre['Movie Count']} movies)")

        # Worst performing genre
        worst_genre = df.iloc[-1]
        print(f"\nLOWEST RATED GENRE:")
        print(f"   {worst_genre['Genre']} - {worst_genre['Avg Rating']}/10 ({worst_genre['Movie Count']} movies)")
    ...
    def analyze_financial_performance(self) -> None:
        ...
        if financial_data:
            ...
            avg_roi = sum(movie['roi'] for movie in financial_data) / len(financial_data)
        ...
        mid_budget = [m for m in financial_data if 50_000_000 <= m['budget'] < 150_000_000]
        ...
        for category_name, category_movies in categories:
            if category_movies:
                ...
                avg_rating = sum(m['rating'] for m in category_movies) / len(category_movies)
                print(f"{category_name}: {len(category_movies)} movies")
                print(f"   Average ROI: {avg_roi:.1f}% | Average Rating: {avg_rating:.1f}/10")
